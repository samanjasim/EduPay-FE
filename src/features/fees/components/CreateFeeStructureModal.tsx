...
import { useCreateFeeStructure } from '../api';
...
export function CreateFeeStructureModal({ isOpen, onClose }: CreateFeeStructureModalProps) {
  ...
  const {
    ...
    defaultValues: {
      name: '', description: null, feeTypeId: '', amount: 0,
      currency: 'IQD', academicYearId: '', frequency: 'OneTime',
      applicableGradeId: null, applicableSectionId: null,
      dueDate: '', lateFeePercentage: 0,
    },
  });
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          ...
          <Controller name="currency" control={control} render={({ field }) => (
            <Select label={t('feeStructures.currency')} options={CURRENCY_OPTIONS} value={field.value} onChange={field.onChange} error={errors.currency?.message} />
          )} />
          <Controller name="frequency" control={control} render={({ field }) => (
            <Select label={t('feeStructures.frequency')} options={frequencyOptions} value={field.value} onChange={field.onChange} error={errors.frequency?.message} />
          )} />
        </div>
